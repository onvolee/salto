...
- [x] panel 打开显示当前 settings 选中的 template name，并按启用字段的 schema order 显示结果。
- [x] 在 settings 选择另一个 template 并 Save 后，下一次打开 panel 使用新 template；当前 panel 不被隐式改写。
- [x] disabled fields 不发起对应工作且不显示为 failed；unavailable/failed sibling 不隐藏 ready sibling。
- [x] close/regenerate/new-open 后返回的旧 response 不会覆盖当前状态。
- [x] active template 删除或损坏时，panel 使用 system-default 并显示可诊断但不泄露 secret 的状态。
...
- 2026-07-19: active-template message/repository/panel tests cover snapshot reads, next-open behavior, schema order, disabled omission, field isolation, durable recovery diagnostics, and stale close/regenerate/new-open responses.
- 2026-07-19: `pnpm --filter @salto/extension test` passed (23 files, 156 tests); extension typecheck and production build passed.
- Browser-level manual acceptance remains for the `ready-for-human` pass.
