...
- [ ] 一个 run 中多个 dictionary fields 只产生一次 Youdao lookup。
- [ ] mixed LLM/dictionary fields 以 template schema order 返回，disabled field 缺席。
- [ ] missing normalized value 显示 unavailable；单个 field 或 provider failure 不隐藏成功 sibling。
- [ ] close/regenerate 后旧 dictionary response 不更新当前 panel。
- [ ] 选择本身和 settings template 保存本身不触发 dictionary request。
