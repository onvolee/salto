...
import { useIsMobile } from "salto-src/hooks/use-mobile"
...
import {
  ...
  SheetHeader,
  SheetTitle,
} from "salto-src/components/ui/sheet"
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            ...
            <SheetDescription>在移动设备上显示设置菜单。</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      ...
      <span className="sr-only">切换设置菜单</span>
    </Button>
  )
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      title="切换设置菜单"
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] after:bg-sidebar-primary/0 after:transition-colors after:duration-150 hover:after:bg-sidebar-primary/40 active:after:bg-sidebar-primary/60 sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
